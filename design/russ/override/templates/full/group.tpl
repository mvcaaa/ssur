...
<div class="content-view-full">
    <div class="class-group">
        ...
        {if eq( ezini( 'group', 'SummaryInFullView', 'content.ini' ), 'enabled' )}
        ...
        {if eq( ezini( 'group', 'ImageInFullView', 'content.ini' ), 'enabled' )}
            {if $node.data_map.image.has_content}
                <div class="attribute-image">
                    {attribute_view_gui attribute=$node.data_map.image image_class=imagelarge alignment=right}
        ...
        {section show=$node.data_map.show_children.content}
            {let page_limit = $node.object.data_map.index_limit.value | or(10)
                 list_items=array()
                 list_count=0}
