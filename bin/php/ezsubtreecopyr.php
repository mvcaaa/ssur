...
$syncNodeIDListSrc[] = $sourceSubTreeMainNode->attribute( 'node_id' );
...
$k = 0;
$counterall = count( $sourceNodeList ); 

while ( count( $sourceNodeList ) > 0 )
{
    if ( $k > $countNodeList )
	{
          list( $column, $counter ) = displayProgress( 'E', $urlImportStartTime, $counter, $counterall, $column );
		  $cli->output("Subtrees not copied: ".var_dump($sourceNodeList));
		  $sourceNodeList=array();
    ...
    for ( $i = 0; $i < count( $sourceNodeList ); $i)
    {
        ...
        else
        {
            ...
            {   // if copying successful then remove $sourceNode from $sourceNodeList
				array_splice( $sourceNodeList, $i, 1 );
//			    eZContentObjectTreeNode::removeSubtrees( array( $sourceObject->attribute('node_id') ), false );
                list( $column, $counter ) = displayProgress( '.', $urlImportStartTime, $counter, $counterall, $column );
//                $cli->output( ".", false );
            }
			else 
			{
				array_splice( $sourceNodeList, $i, 1 );
                list( $column, $counter ) = displayProgress( 'S', $urlImportStartTime, $counter, $counterall, $column );
                $cli->output( "Check ".$sourceNodeID, false );
//		        $cli->error( "Subtree Copy Error!\nToo many loops while copying nodes." );
//		        $script->shutdown( 6 );
//				$i++;
			}
...
$newSubTreeMainNodeID = $dstNodeID;
//	$syncNodeIDListSrc[ $key ];
